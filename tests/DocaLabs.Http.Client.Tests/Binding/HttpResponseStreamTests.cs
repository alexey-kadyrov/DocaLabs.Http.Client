...
namespace DocaLabs.Http.Client.Tests.Binding
{
    ...
    [Subject(typeof(HttpResponseStream))]
    class when_http_response_is_used_asynchronously_with_byte_array_data : response_deserialization_test_context
    {
        Establish context =
            () => SetupAsync("application/octet-stream", new MemoryStream(Encoding.UTF8.GetBytes("Hello World!")));

        It should_return_all_byte_array_data =
            () => Encoding.UTF8.GetString(http_response_stream.AsByteArrayAsync(CancellationToken.None).Result).ShouldEqual("Hello World!");
    }
    ...
    [Subject(typeof(HttpResponseStream))]
    class when_asynchronously_reading_with_plain_text_data_with_defined_charset_as_string : response_deserialization_test_context
    {
        Establish context =
            () => SetupAsync("text/plain; charset=utf-8", new MemoryStream(Encoding.UTF8.GetBytes("Hello World!")));

        It should_deserialize_string_data =
            () => http_response_stream.AsStringAsync().Result.ShouldEqual("Hello World!");
    }
    ...
    [Subject(typeof(HttpResponseStream))]
    class when_asynchronously_reading_with_plain_text_data_without_defined_charset_as_string : response_deserialization_test_context
    {
        Establish context =
            () => SetupAsync("text/plain", new MemoryStream(Encoding.GetEncoding("ISO-8859-1").GetBytes("Hello World!")));
    ...
    [Subject(typeof(HttpResponseStream))]
    class when_asynchronously_reading_with_plain_text_data_without_defined_charset_as_string_and_overriding_encoding : response_deserialization_test_context
    {
        Establish context =
            () => SetupAsync("text/plain", new MemoryStream(Encoding.UTF32.GetBytes("Hello World!")));

        It should_deserialize_string_data_using_provided_encoding =
            () => http_response_stream.AsStringAsync(Encoding.UTF32).Result.ShouldEqual("Hello World!");
    }
    ...
    [Subject(typeof(HttpResponseStream))]
    class when_asynchronously_reading_with_plain_text_data_with_defined_charset_as_string_using_specific_matching_encoding : response_deserialization_test_context
    {
        Establish context =
            () => SetupAsync("text/plain; charset=utf-32", new MemoryStream(Encoding.UTF32.GetBytes("Hello World!")));
    ...
    [Subject(typeof(HttpResponseStream))]
    class when_checking_asynchronous_web_request_properties_on_http_response_stream
    {
        static Mock<WebRequest> mock_request;
        static Mock<WebResponse> mock_response;
        static HttpResponseStream http_response_stream;
    ...
    [Subject(typeof(HttpResponseStream))]
    class when_using_stream_methods_and_properties_on_asynchronous_http_response_stream
    {
        static Mock<WebRequest> mock_request;
        static Mock<WebResponse> mock_response;
        static HttpResponseStream http_response_stream;
        static Mock<Stream> mock_underying_stream;
        static Mock<IAsyncResult> async_result;
        static MemoryStream destination_stream;
        static byte[] buffer;
        ...
        Establish context = () =>
        {
            ...
            async_result = new Mock<IAsyncResult>();

            destination_stream = new MemoryStream();
...
#pragma warning disable 168
            ...
            http_response_stream.FlushAsync(CancellationToken.None);

            http_response_stream.ReadAsync(buffer, 666, 777, CancellationToken.None);
            http_response_stream.BeginRead(buffer, 666, 777, null, null);
            http_response_stream.EndRead(async_result.Object);

            http_response_stream.WriteAsync(buffer, 888, 999, CancellationToken.None);
            http_response_stream.BeginWrite(buffer, 666, 777, null, null);
            http_response_stream.EndWrite(async_result.Object);

            http_response_stream.CopyToAsync(destination_stream, 4096, CancellationToken.None);
...
#pragma warning restore 168
        ...
        It should_call_asynchronously_flush_on_the_undelying_stream =
            () => mock_underying_stream.Verify(x => x.FlushAsync(CancellationToken.None), Times.AtLeastOnce());

        It should_call_read_async_on_the_undelying_stream =
            () => mock_underying_stream.Verify(x => x.ReadAsync(buffer, 666, 777, CancellationToken.None), Times.AtLeastOnce());

        It should_call_begin_read_on_the_undelying_stream =
            () => mock_underying_stream.Verify(x => x.BeginRead(buffer, 666, 777, null, null), Times.AtLeastOnce());

        It should_call_end_read_on_the_undelying_stream =
            () => mock_underying_stream.Verify(x => x.EndRead(async_result.Object), Times.AtLeastOnce());

        It should_call_write_async_on_the_undelying_stream =
            () => mock_underying_stream.Verify(x => x.WriteAsync(buffer, 888, 999, CancellationToken.None), Times.AtLeastOnce());

        It should_call_begin_write_on_the_undelying_stream =
            () => mock_underying_stream.Verify(x => x.BeginWrite(buffer, 666, 777, null, null), Times.AtLeastOnce());

        It should_call_end_write_on_the_undelying_stream =
            () => mock_underying_stream.Verify(x => x.EndWrite(async_result.Object), Times.AtLeastOnce());

        It should_call_copy_async_on_the_undelying_stream =
            () => mock_underying_stream.Verify(x => x.CopyToAsync(destination_stream, 4096, CancellationToken.None), Times.AtLeastOnce());
    }
}
