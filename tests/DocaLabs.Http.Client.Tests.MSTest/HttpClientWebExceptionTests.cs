...
namespace DocaLabs.Http.Client.Tests.MSTest
{
    ...
    public class HttpClientWebExceptionTests
    {
        ...
        [TestMethod]
        public void WhenHttpClientWebExceptionIsSerialized()
        {
            using (ShimsContext.Create())
            {
                var lastModified = DateTime.Now;
                ...
                var innerException = new WebException("Failed request", null, WebExceptionStatus.ProtocolError,
                    shimHttpWebResponse);

                var originalException = new HttpClientWebException("Request Failed", innerException);
                ...
                var targetException = (HttpClientWebException) formatter.Deserialize(stream);


                Assert.IsNotNull(targetException.InnerException);
                Assert.AreEqual("Failed request", targetException.InnerException.Message);
